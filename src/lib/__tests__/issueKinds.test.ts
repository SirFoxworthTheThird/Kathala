...
/*
  ...
  35 of 50 "warnings" were of that shape — nineteen dangling or dormant
  subplots, thirteen faction gaps, twelve long runs in one point of view (a
  check since removed outright) — every one of them Dumas writing a novel. Drawn and ranked like a real fault, they teach the writer to
...
describe('notes rank below warnings', () => {
  ...
  it('does not let a note drag a group below its worst member', () => {
    ...
    const groups = groupIssuesByKind([
      issue('a', 'thread-dangling', 'note'),
      issue('b', 'thread-dangling', 'warning'),
    ])
